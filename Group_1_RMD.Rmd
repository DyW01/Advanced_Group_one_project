...
tidy_data <- tidy_data %>%   #Assign our transformed data to object
  ...
  mutate(sex = as.factor(sex)) %>%         #Defining column content as categorical
...
```{r Numeric to categorical (factor)}
# Converts continuous numeric values into categorical (factor) data
# This methods allows the conversion of several variables into once
factor_cols <-  c("sex", "year", "abm", "race", "blood_neut_pct")
tidy_data[, factor_cols] <- lapply(tidy_data[, factor_cols] , factor)
